...
The first off-board replica runs on `dune-fd-test01.fnal.gov` under:

```text
/storage/gizmo-monitor
...
The board remains on its isolated `192.168.191.53` maintenance link. A
...
```sh
ssh arroyave@dune-fd-test01.fnal.gov \
  /storage/gizmo-monitor/bin/status
```

The SQLite database is:

```text
/storage/gizmo-monitor/data/gizmo-history.sqlite3
```
...
events even when the workstation relay or `dune-fd-test01` is unreachable.
...
`dune-fd-test01` does not grant the operator passwordless administration and
