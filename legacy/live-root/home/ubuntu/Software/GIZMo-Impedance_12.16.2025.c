...
double phaseArrayR[64];
double phase2ArrayR[64];
...
{
    LoadTH("/home/ubuntu/Software/setThreshold.env");
    int memfd;
    //char str[255];
    char cmd[200];
...
void read_gpio_1()
{
    ...
    int array[8];  // Array to store each digit of 'i'
    for (int j = 0; j < 8; j++) { 	
...
void parse(const char *cmd)
{
    ...
    //---------------------------- END SOCKET SETUP ---------------------
        ...
        unsigned long num_Reads = 100;
        //int i = 0;
        double ave = 0;
        double aveI = 0;
        double aveQ = 0;
        double initialRun = 0;
        int capacitanceAlarm = 0;
        double maxMeasuredMag = 19000;
        int normalizeFlag = 0;
        //unsigned long value [16384];
        relay_state = 0xA5556AAA;
        write_gpio_1(OFFSET, relay_state);
        LoadResistance("/home/ubuntu/Software/resistance.env");
        LoadCapacitance("/home/ubuntu/Software/capacitance.env");
        Load_auto();
        ...
        if (sscanf(cmd + 4, "%ld", &num_Reads) > 0)
        {  
            while (1 > 0)            
            {
                readFromLatch();
                ...
                else if (x_reverse < global_th)
                {
                    ...
                    if (latched == 0) {
                        ...
                        latched = 1;
                        strncpy(latchedStamp, bufferTime, sizeof(latchedStamp) - 1);
...
//------------------------------------------This is I, Q and Mag-----------------------
    ...
    if (!strncmp(cmd, "CAL", 3))
    {
        ...
        if (sscanf(cmd + 4, "%ld", &num_Reads) > 0)
        {
            ...
            double total_resistance[128];
            double total_capacitance[128];
            ...
            for (int p = 0; p < 65; p++) {
                ...
                total_res[p+1] = total_resistance[p];
            ...
            for (int p = 0; p < 128; p++){
                ...
                double aveQ = 0;
                double phase_rad, phase_rad2, phase_deg, phase_deg2;
                total_cap[p+1] = total_capacitance[p];
...
void Load_auto()
{
    FILE *file = fopen("/home/ubuntu/Software/Rcalibration_ph.csv", "r");
    if (file == NULL)
    {
    ...
    while (index < 65 && !feof(file))
    {
        
        if (fscanf(file, "%lf,%lf,%lf,%lf", &res, &mag, &phase, &phase2) == 4)
        {
            total_res[index] = res;
            magArrayR[index] = mag;
...
int WriteADCtoFile(unsigned long ADC[], int digitsToWrite){
    ...
    FILE *fp = fopen("/home/ubuntu/Software/adc.csv", "w");
    if (fp == NULL)
    {
    ...
    for (unsigned long i = 0; i < digitsToWrite; i++)
    {
        fprintf(fp, "%lu, %f, %f\n", ADC[i], _fI[i], _fQ[i]);
    }
...
int WriteCalibrationValuesR(){
    ...
    FILE *fp = fopen("/home/ubuntu/Software/Rcalibration.csv", "w");
    if (fp == NULL)
    {
...
int WriteCalibrationValuesR_ph(){
    // Write the values from calibration to a file
    FILE *fp = fopen("/home/ubuntu/Software/Rcalibration_ph.csv", "w");
    if (fp == NULL)
    {
        perror("Failed to open Rcalibration_ph.csv");
        return 1;
...
int WriteCalibrationValuesC(){
    // Write the values from calibration to a file
    FILE *fp = fopen("/home/ubuntu/Software/Ccalibration.csv", "w");
    if (fp == NULL)
    {
...
int WriteCalibrationValuesC_ph(){
    // Write the values from calibration to a file
    FILE *fp = fopen("/home/ubuntu/Software/Ccalibration_ph.csv", "w");
    if (fp == NULL)
    {
...
double interpolate_x(double y_val, int max_iter, double tol) {
    ...
    for (int i = 0; i < nTerp - 1; i++) {
        ...
        if ((y_val >= y0 && y_val <= y1) || (y_val <= y0 && y_val >= y1)) {
            ...
            double low = total_res[i], high = total_res[i + 1], mid;
...
void readFromLatch() {
    FILE *file = fopen("/home/ubuntu/Software/latchState.env", "r");
    if (file == NULL) {
    ...
    if (fgets(line, sizeof(line), file)) {
        strncpy(latchedStamp, line, sizeof(latchedStamp) - 1);
        // Remove newline if present
        latchedStamp[strcspn(latchedStamp, "\r\n")] = '\0';
    }
...
void writeToLatch(int latchedValue, const char *timestamp) {
    FILE *file = fopen("/home/ubuntu/Software/latchState.env", "w");
    if (file == NULL) {
...
double findCenterOfCap(double array_x[], double array_y[]) {
    ...
    //get the mean using last 8 points to the left and next 8 points to the right of the maximum
    double sum = 0;
    for (int i = (maxIndex - 8); i < (maxIndex + 8); i++){
        sum += array_y[i];
    }
    double mean = sum / 16;
    ...
    double minRight = mean;
    int minRightIndex;
    double localRight;
    for (int i = (maxIndex + 1); i < maxIndex + 9; i++){
        localRight = mean - array_y[i];
        if (abs(localRight) < minRight){
            minRight = abs(localRight);
            minRightIndex = i;
        }
    }
...
int finxMaxMagnitude(void) {
    int max = magArrayR[0]; //initial array value
...
int readNormalizeMagFlag()
{
    FILE *file = fopen("/home/ubuntu/Software/normalizeMagFlag.env", "r");
    if (file == NULL)
    {
...
int writeNormalizeMagFlag(int flag)
{
    ...
    FILE *file = fopen("/home/ubuntu/Software/normalizeMagFlag.env", "w");  // Open for writing (overwrite)
    if (file == NULL)
    {
